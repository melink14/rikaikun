...
window.rcxMain = {
  ...
  _onLoad: function() {
    try {
      //this.loadPrefs();
      //this.haveNames = this.canDoNames = (typeof(rcxNamesDict) !=
  'undefined');
    }
    catch (ex) {
    ...
    try {
      ...
      for (i = 1; i >= 0; --i) {
        ...
        try {
          ...
          document.getElementById('rikaichan-separator-' + xm[i]).hidden = a ||
  b;
        }
        catch (ex) {
      ...
      this.cfg.css = (this.cfg.css.indexOf('/') == -1) ?
  ('chrome://rikaichan/skin/popup-' + this.cfg.css + '.css') : this.cfg.css; if
  (!this.isTB) { for (i = 0; i < gBrowser.browsers.length; ++i) { c =
  gBrowser.browsers[i].contentDocument.getElementById('rikaichan-css'); if (c)
  c.setAttribute('href', this.cfg.css);
        }
      }
...
*/
  ...
  showDownload: function() {
    ...
    try {
      ...
      if ((typeof(rcxWordDict) != 'undefined') && (rcxWordDict.version !=
  null)) { u += 'wv=' + rcxWordDict.version + '&';
      }
      if ((typeof(rcxNamesDict) != 'undefined') && (rcxNamesDict.version !=
  null)) { u += 'nv=' + rcxNamesDict.version + '&';
      }
      if (u.length) u = url + '?' + u;
        else u = url;
...
*/
  ...
  savePrep: function(clip, entry) {
    ...
    if (clip) {
      ...
      // mk = this.cfg.smaxck; // something related to the number of kanji in
      // the look-up bar
    }
    /* else { // save to file
      me = this.cfg.smaxfe;
    ...
    if (rcxMain.config.lineEnding == 'rn')
      text = text.replace(/\n/g, '\r\n');
    else if (rcxMain.config.lineEnding == 'r')
      text = text.replace(/\n/g, '\r');
  ...
  inlineToggle: function(tab) {
    if (rcxMain.enabled)
      rcxMain.inlineDisable(tab, 1);
    else
      rcxMain.inlineEnable(tab, 1);
  },
