...
*/
	...
	savePrep: function(clip, entry) {
		...
		if (rcxMain.config.lineEnding == 1) text = text.replace(/\n/g, '\r\n');
			else if (rcxMain.config.lineEnding == 2) text = text.replace(/\n/g, '\r');
		if (rcxMain.config.copySeparator != '\t') return text.replace(/\t/g, rcxMain.config.copySeparator);
